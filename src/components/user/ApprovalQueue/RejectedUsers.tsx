...
export const RejectedUsers: React.FC = () => {
  ...
  useEffect(() => {
    dispatch(fetchUsers(constants.USER.STATUS.REJECTED));
  ...
  const fetchingRejectedUsers = useSelector(getFetchingUsers);
  const queueErrorMessage = useSelector(getFetchUsersErrorMessage);
  const updateUsers = useSelector(getUpdateUsers);
  const rejectedUsers = useSelector(getUnapprovedUsers);
  ...
  const approveBtnHandler = (index: number) => {
    dispatch(
      setUserApproval(rejectedUsers[index], {
        index: index,
        approved: true,
        rejected: false,
      })
  ...
  if (userAccessLevel !== constants.USER.ACCESS_LEVELS.ROOT) {
    ...
    return (
      <IonCard>
        <IonCardHeader>
          <IonCardTitle>All Rejected Users</IonCardTitle>
        </IonCardHeader>
        <IonCardContent>
          <IonGrid>
            <IonRow>
              {fetchingRejectedUsers ? (
                <IonProgressBar type="indeterminate"></IonProgressBar>
              ) : (
                ""
              )}
              {rejectedUsers.length > 0
                ? rejectedUsers.map((unvUser, index) => {
                    return (
                      <IonCol key={index} size={"4"}>
                        <IonCard>
                          ...
                          <IonCardContent>
                            ...
                            <IonText style={TextContainerStyle} color="danger">
